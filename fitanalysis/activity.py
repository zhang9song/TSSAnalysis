...
class Activity(fitparse.FitFile):
    ...
    for record in records:
      ...
      if not excise or DEBUG_EXCISE:
        ...
        for rec_field in record.fields:
          if rec_field is not None and rec_field.value is not None:
            new_fields.append(rec_field)
        ...
        for field_name in fields:
          ...
          if field is not None or field.value is not None :
  ...
  def _detect_start_stop_events(self, records):
    ...
    for i, record in enumerate(records):
      ...
      elif record.get('speed').value is not None and record.get('speed') is not None:
  ...
  def intensity(self, ftp):
    ...
    Intensity factor is defined as the ratio of normalized power to FTP.txt.
